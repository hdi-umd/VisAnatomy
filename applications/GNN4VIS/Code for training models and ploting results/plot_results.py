...
def plot_results(res_dfs):
    ...
    for acc_key in acc_keys:
        figure, axis = plt.subplots(1, 1, figsize=(10, 6))  # Increased figure size
        default_palette = sns.color_palette()
        
        lines = []
        for i, (key, df) in enumerate(res_dfs.items()):
            color = default_palette[i]
            line = sns.lineplot(data=df, x="ts", y=acc_key, 
                                errorbar='sd', ax=axis, label=key, 
                                color=color, linewidth=2)
            lines.append(line)

        # Adjust plot limits
        x_max = max(df['ts'].max() for df in res_dfs.values())
        y_min, y_max = axis.get_ylim()
        axis.set_xlim(0, x_max * 1.15)  # Extend x-axis by 10%
        axis.set_ylim(y_min, y_max + (y_max - y_min) * 0.1)  # Extend y-axis by 10%

        # Add labels at the end of each line
        for i, (line, (key, df)) in enumerate(zip(lines, res_dfs.items())):
            last_y = df[acc_key].iloc[-1]
            corr = {"Model 1": "Graph 1", "Model 2": "Graph 2", "Model 3": "Graph 3", "Model 4": "Graph 4", "Model5": "Img-Only", "Model 6": "Img+Graph 4"}
            axis.text(x_max * 1.02, last_y, corr[key], color=default_palette[i], 
        ...
        plt.tight_layout()
        plt.savefig(f"{current_dir}/figures/{acc_key}.pdf", bbox_inches="tight", dpi=300)
        plt.close()
