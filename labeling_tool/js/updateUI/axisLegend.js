function addAxisConfiguration(forceIndex) {
  ...
  //console.log("adding axis configuration for index", index, VA.axes.length);
  ...
  const axisDiv = createAxisElement(axisCount);
  ...
  document
    ...
    .insertBefore(axisDiv, document.querySelector(".refElemDiv:last-child"));

  // Populate the column dropdown for the new axis if CSV data is available
  if (VA.csvColumns) {
    populateColumnSelect(`columnSelect_${axisCount}`, undefined);
    // populateColumnDropdowns();
    // if (VA.csvColumns.length > 0) {
...
function populateColumnSelect(id, selectedVal) {
  const attrSelect = d3.select("#" + id);
  if (!VA.dataset) {
  ...
  attrSelect.selectAll("option.header-option")
    ...
    .text(d => d);
  // Set selected value
  attrSelect.property("value", selectedVal);
}
